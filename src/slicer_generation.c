...
SURF_NORMAL_FUNC *surf_normal_func, CRITICAL_XY_FUNC *critical_xy_func, double cv, int surface_type) {
...
void GetSliceParamsStandard(double* alpha, double* beta, double* gamma, double* cv, double* k, int slice_num, int col_num, IMAGE_SLICER_PARAMS p) {
    ...
    *cv = p.cv; *k = p.k;
    ...
    int row_num = floor( slice_num / p.n_each );
...
double ImageSlicerSag(double x, double y, IMAGE_SLICER_PARAMS p, double custom_slice_params[]) {
    ...
    GetSurfaceFuncs(&sag_func, &transfer_dist_func, &surf_normal_func, &critical_xy_func, cv, p.surface_type);
